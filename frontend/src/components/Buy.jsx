...
const Buy = () => {
   ...
   useEffect(() => {
    ...
    const res = await axios.get("http://localhost:4000/course/detail/" + courseId.id, {
  ...
  return (
     <div>
     <div className='fixed top-0 left-0 w-full z-2 bg-white shadow-2xl '> <Navbar2/></div>
    <div className="flex relative mt-13">
     <div className='min-h-screen bg-[#e5eeff]  fixed  left-0 w-60 '>
      <div className='m-10 text-gray-500'>
       <div className='flex flex-col justify-between space-y-7 cursor-pointer'>
        ...
        <div onClick={()=>{localStorage.removeItem("token"); localStorage.removeItem("user");  navigate("/")}}>Logout</div>
         </div>
        </>}
     ...
     <div className='ml-60 relative bg-white w-full h-screen'>
      <div className='bg-blue-700 relative h-80 w-full '>
        <div className=' text-center h-40 w-100 left-30  absolute top-25 text-white font-bold text-4xl wrap-break-word '>{course.title}</div>
      ...
      <div className='relative'><span className='absolute top-10 left-20 text-2xl font-bold text-blue-600 underline underline-offset-12'>Overview</span></div>
     <hr class=" border-gray-300  w-140 m-20"/>
