...
adminRoutes.post("/orders", adminMiddleware, async (req, res) => {
  ...
  console.log("price of the course is ", Price);
  const options = {
  ...
  if (sellerId.toString() === buyerId.toString()) {
    return res.json({ message: "your are creator so you can't buy own" });
  }
...
adminRoutes.post("/payment-verification",adminMiddleware,async (req,res)=>{
  const buyerId = req.adminId;
  const courseId = req.body.courseId;
  const course = await courseModel.findById(courseId);
  const sellerId = course.creatorId;
  
  console.log("this is coming after payment success ",req.body)
...
//  console.log("coming sign",razorpay_signature);
...
adminRoutes.get(
  ...
  async function (req, res) {
    const adminId = req.adminId;
    const ordersDetails = await orderModel.find({ buyerId: adminId });
    const OrdersWithCourseinfo = await orderModel
      .find({ buyerId: adminId })
      .populate("courseId");
    console.log("in backend code purchased courses ", ordersDetails);
    const courseData = OrdersWithCourseinfo.map((key) => key.courseId);
    console.log("in backend code coursesoforders ", courseData);

    res.json({
      message: "all course",
      ordersDetails: ordersDetails,
      ListofProducts: courseData,
      OrdersWithCourseinfo: OrdersWithCourseinfo,
    });
  }
);
